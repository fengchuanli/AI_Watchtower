...
│   ├── monthly-continuity-snapshot.md
...
- 月度连续观察快照：`docs/monthly-continuity-snapshot.md`
...
每次抓取或优化都应该留下清楚的履历，并推送到 GitHub。提交标题必须区分 `【新闻更新】`、`【VisionHub网站风格优化】`、`【网站优化】`，其中 `【VisionHub网站风格优化】` 优先于普通 `【网站优化】`：只要改到首页结构、UI、手机端、卡片层级、详情页叙事、排行榜表现或视觉简报，就用 VisionHub 分类。优化履历必须写清楚 `网站可见变化`，也就是用户能在首页、新闻流、详情页、全部新闻或标签页哪里看到变化。定时任务异常时先检查 `docs/automation-health-check.md`，尤其确认 `ACTIVE` 任务没有过期的 `UNTIL` 日期。
