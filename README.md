...
每次抓取或优化都应该留下清楚的履历，并推送到 GitHub。提交标题必须区分 `【新闻更新】`、`【网站优化】`、`【VisionHub网站风格优化】`，优化履历必须写清楚 `网站可见变化`，也就是用户能在首页、新闻流、详情页、全部新闻或标签页哪里看到变化。定时任务异常时先检查 `docs/automation-health-check.md`，尤其确认 `ACTIVE` 任务没有过期的 `UNTIL` 日期。
