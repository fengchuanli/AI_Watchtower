...
- For a batch of semi-automated candidates, write the discovered `title`, `sourceUrl`, optional `publishedAt`, and any one-sentence `sourceBackedFact` values into a temporary JSON file and run `node scripts/report-duplicate-candidates.mjs <candidate-file.json>` before drafting. Interpret the report with `docs/candidate-intake-format.md` duplicate statuses: `repeated-url` means the same source URL is already current, archived, or in the same candidate batch; `near-title-review` means similar wording needs manual fact comparison; `fresh-source-fact` is allowed only when the new source action is named in `sourceBackedFact`; `manual-clear` means no script or manual current/history match remains. Treat unresolved repeated URLs or near-title matches as `hold-duplicate-review`. If no fresh fact exists, use `reject-repeated-source-fact`.
- Use the report's `Review actions` lines as the first draft of the editor decision: repeated URLs reject by default unless the same page was materially updated; near-title matches hold by default until the editor names the exact source-backed fact; fresh-source clearance requires a new official announcement, filing, audit result, model card, customer metric, regulator text, paper revision, or independent benchmark, not just a differently worded headline.
