...
Use this checklist after a candidate has passed `docs/candidate-source-checklist.md`, has a completed `docs/candidate-intake-format.md` record, has resolved any media-to-original-source decision with `docs/original-source-replacement-guide.md`, has been ranked with `docs/candidate-priority-rubric.md`, and has passed any needed batch-level source-diversity triage in `docs/source-diversity-triage-note.md`. Pair it with `docs/detail-page-review-guide.md` so technical claims on the public detail page become fact, impact, boundary, and next-check blocks instead of source-article replacement copy. Use `docs/counter-evidence-observable-guide.md` when `counterEvidence` needs a downgrade signal that names a source artifact or measurable result. Pair it with `docs/update-run-checklist.md` so the whole news update also records source discovery, duplicate reporting, homepage preflight from `docs/homepage-edition-preflight.md`, partial-batch decisions from `docs/partial-batch-publication-guide.md`, validation, commit, and push status. After the current edition is drafted, apply `docs/homepage-edition-preflight.md` before `docs/current-to-history-publication-checklist.md` so the homepage reader question, TOP3 use, source boundary, mobile scan path, and proof boundary are settled before the newest archive edition mirrors the same reader frame, source boundary, item count, and item order. Its job is to turn one approved `draft` candidate into `data/news.json` without copying source article text or losing the editorial boundaries recorded during intake.
...
4. Write `nextCheck`, `evidenceThreshold`, `followUpQuestions`, and `counterEvidence` from `nextIndependentCheck` plus the observable downgrade signal. Use `docs/counter-evidence-observable-guide.md` when deciding whether `counterEvidence` should name a source artifact or a measurable result.
