...
Each future news-intelligence run should aim for at least 10 qualified `items` so the homepage news flow has enough non-TOP3 stories to scan. This is a target, not permission to lower quality: duplicate, paywall/login-wall, unclear-source, weak-proof, or copyright-risk candidates must still be skipped. If a run publishes fewer than 10 safe items, record the reason in the edition metadata and optimization log. Use a compact, non-apologetic short-batch note in `edition.editorialInterpretation`: state `本期发布 X 条安全非重复信号`, say `少于 10 条是质量门槛结果`, and name the unsafe padding types that were not used, such as old articles, podcasts, paywall/login-wall body text, community discussion, duplicates, weak-proof items, or routine marketing. Avoid wording such as "只发布", "不足", "遗憾", "来不及", or "未完成", which makes a deliberate quality gate sound like an incomplete run. When only one or two reliable candidates remain, use `docs/partial-batch-publication-guide.md` before drafting so the run explicitly chooses to publish a partial batch, continue searching, or hold the update.
...
- `topicContinuity`: At least two compact current-edition notes for recurring topics. Each object needs `topic`, `label`, `status`, `previousPattern`, `currentSignal`, `signalDirection`, and `stillUnproven`. `topic` must match a current `topicGroups[].id`, and `status` must be `stronger`, `weaker`, or `repeated`. Use `docs/topic-continuity-review-note.md` before writing this field so readers can tell whether a recurring theme has stronger evidence, weaker evidence, or is merely repeating; do not introduce new source facts beyond current and archived edition context. Do not mark a topic `stronger` only because several media reports, newsletters, podcasts, or secondary summaries repeat the same storyline; repeated coverage is `repeated` unless the current source changes the artifact, actor, scope, metric, date, or accountable owner. `stillUnproven` must name the official, original, filing, contract, log, audit, metric, regulator, data, report, or third-party proof needed next.
- `companyContinuity`: At least two compact current-edition company notes for recurring companies. Each object needs `company`, `label`, `lastMention`, `whatChanged`, and `stillUnproven`. Use this for company-level continuity only when the company appears in current `items[].companies`; `lastMention` should name the previous or historical context, `whatChanged` should state what this edition adds, downgrades, repeats, or resolves after applying `docs/company-continuity-review-note.md`, and `stillUnproven` must say what remains unproven while naming the official, original, filing, contract, log, audit, metric, regulator, data, or third-party proof needed next. Do not use this field to introduce new source facts, and do not treat repeated company mentions as stronger evidence unless the current source changes the artifact, actor, scope, metric, date, or accountable owner.
...
Homepage topic groups also render planned-topic why-now summaries and omission boundaries for supported topics. Each supported topic should have a compact `whyNow` framing line explaining why Agent, model, enterprise, policy, infrastructure, or developer-tooling signals matter in the current AI cycle. Each omitted topic should include an explicit omission status such as `无新来源事实`, a short reason, a `omissionBoundary` line that says the omission does not mean the topic is unimportant, the evidence that would make it promotable, and where readers can look next. Omitted-topic `fallback` copy should point readers to the archive, tag page, historical context, or current already-selected related topics as background only; it must not imply a fresh source fact, confirmed landing, or all-market conclusion. Keep this wording editorial and source-boundary focused; do not use topic context or omitted-topic copy to introduce new factual claims.
...
- `editions[].topicContinuity`: Copy of the latest current edition's recurring-topic continuity notes, including whether each topic signal is stronger, weaker, or repeated after applying `docs/topic-continuity-review-note.md`, so archive readers keep the same evidence-strength framing.
...
Company tag pages derive OpenAI, Anthropic, Google, and Meta views from historical `sourceId`, `source`, and `sourceUrl` fields. Each tag context should show the latest matched signal, the last-seen edition date, and a source caveat derived from `claimBoundary`, `provenance`, or `nextCheck`, so company pages remain a tracking aid rather than a new source of unverified claims. When a later edition mentions the same company again, use `docs/company-continuity-review-note.md` to decide whether the latest signal is stronger, weaker, repeated, or resolved before turning archive context into public `companyContinuity` copy. When a later edition mentions the same topic again, use `docs/topic-continuity-review-note.md` so repeated media or vendor narratives stay background context unless the current source adds a stronger evidence object.
...
- `followUpQuestions`: At least two concrete Chinese questions ending in `？` that guide the next editorial update or source check. Each question should name the source artifact, observable result, or status change the next editor should check, such as a court record, filing, model card, price page, code release, dataset, benchmark setup, audit sample, deployment log, customer metric, regulator text, or official response. Avoid generic templates like `是否已有独立来源确认[公司]的核心事实？`, `下一版能否找到客户日志、官方文件、论文复现或监管材料？`, or bare `继续观察` wording because they do not tell editors where to look first.
- `evidenceThreshold`: Concrete minimum evidence required before the observation can be upgraded to a confirmed editorial judgment. Name the source artifact or proof type needed, and say what verified editorial status it would unlock, such as moving from a media signal to a confirmed organization change, fact conclusion, verified policy, or validated deployment. Common patterns: media signal -> official announcement, filing, regulator text, company statement, court record, or named organization document before upgrading to confirmed organization/policy/transaction fact; vendor claim -> customer-side metric, signed contract, deployment log, third-party benchmark, audit material, external expert review, or regulator filing before upgrading to independently verified deployment, performance, cost, or safety claim; research preprint -> peer review, independent replication, released dataset/code, third-party benchmark rerun, safety evaluation, or real-world transfer test before upgrading to replicated research result or deployable capability evidence. Avoid vague phrases such as "more evidence", "等待更多证据", or "继续观察" unless the sentence also names the exact artifact and status change.
...
- `counterEvidence`: Short Chinese condition explaining what later evidence would weaken, downgrade, or narrow the current editorial judgment. For promoted current items and the latest archived snapshot, use `docs/counter-evidence-observable-guide.md` to decide whether the field should name a source artifact or a concrete observable outcome, and whether the result should `降级/下调` the story or only `收窄/限定` its scope. Downgrade when later evidence attacks the central claim, such as official denial, missing contract or filing, failed replication, cancelled delivery, invalid legal basis, or negative measured effect; name the weaker replacement status. Narrow when the core fact still stands but applies to fewer regions, customers, tasks, dates, features, or deployment contexts; name the smaller boundary. Prefer observable outcomes when the claim is about adoption, deployment, safety, performance, cost, or policy effect; examples include access logs, approval records, adoption metrics, third-party retests, product delivery status, customer-side task results, error rates, latency/cost records, audit findings, regulator enforcement, or user impact. Use documents such as policy text, official announcements, filings, contracts, papers, model cards, system cards, or role confirmations when the downgrade depends on whether a stated fact exists. Avoid vague phrasing like "更多证据".
- `source`: Human-readable source group.
- `sourceName`: Preferred human-readable source name for cards and detail pages. During migration, `source` remains the fallback. Keep this as the owner or source-family label; put the exact supported fact in `provenance` so the detail-page source panel does not show a bare source directory entry.
...
- `provenance`: How readers should interpret the source and claim quality. It must name the exact source fact the detail-page source reference supports, including an action and object such as `发布 TimesFM-3`, `报道数据中心政治动员`, `宣布 Ads Manager 区域扩展`, or `讨论 Agent 执行治理`. For media sources, it should also remind readers that full facts, quotes, charts, interviews, and context belong in the original source. Avoid generic labels such as `VentureBeat AI 只支持最小报道事实`, `官方来源`, `source link`, or `original article` when they do not say which fact the link supports.
...
For VisionHub-style narrative events, source role matters more than story appeal. Use `厂商主张` for company-written policy proposals, customer stories, benchmark narratives, or other vendor-framed pages unless the linked page directly verifies a stronger category such as an official release fact, research original, or regulator action. A `厂商主张` item may be marked `已核验` only for the narrow fact that the vendor made the claim or proposal; `provenance`, `claimBoundary`, `nextCheck`, `evidenceThreshold`, and `counterEvidence` must name the missing external proof so readers do not mistake a vivid narrative for independent confirmation. Before promoting one of these items into TOP3 or the latest archive snapshot, apply `docs/vendor-narrative-promotion-rule.md`: at least one first-screen reason field such as `whyItMatters`, `whyRanked`, `topReason`, or `readerUse` must name the independent proof path, so the card itself does not read like vendor confirmation. The validator checks both `nextCheck` and promoted first-screen copy because they are the earliest follow-up cues readers see on homepage cards and detail pages.
